				...
				<table>
					...
					<tbody>
						<c:forEach items="${membermenu}" var="membermenu">
							<tr>
								<td>
									<input type="checkbox" name="RowCheck" class="RowCheck" value="${membermenu.userId}">
								</td>
								<td>
									<c:out value="${membermenu.userName}" />
								</td>
								<td>
									<a href="/admin/AdminMemberUpdate?userId=${membermenu.userId}">
										<c:out value="${membermenu.userId}"></c:out>
									</a>
								</td>
								<td>
									<c:out value="${membermenu.phone}" />
								</td>
								<td>
									<c:out value="${membermenu.email}" />
								</td>
								<td  style="width:450px;">
									<c:out value="${membermenu.addressNum}" />&nbsp;
									<c:out value="${membermenu.address}" />&nbsp;
									<c:out value="${membermenu.addressDetail}" />
								</td>
								<td>
									<fmt:formatDate value="${membermenu.regDate}" pattern="yyyy년MM월dd일 HH시mm분" />
								</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				<input type="button" value="삭제" class="deletebutton button_style delete_btn" onclick="deleteValue();">	
				...
				<div class="search_wrap">
					<div class="search_area">
						<select name="type" class="text-frame text-frame-margin">
							...
							<option value="I"
								<c:out value="${pageMaker.cri.type eq 'I'?'selected':'' }"/>>회원 ID</option>
							<option value="N"
								<c:out value="${pageMaker.cri.type eq 'N'?'selected':'' }"/>>회원명</option>
							<option value="D"
								<c:out value="${pageMaker.cri.type eq 'D'?'selected':'' }"/>>가입일</option>
						</select> 
						<input type="text" name="keyword" class="text-frame"value="${pageMaker.cri.keyword }">
